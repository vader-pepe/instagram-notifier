...
def get_otp(url: str) -> str:
    ...
    # Raises an HTTPError if the response code was unsuccessful.
...
def get_post(username: str = Query(..., description="Instagram username to fetch posts from")):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving posts: {e}")
    ...
    for media in medias:
        ...
        taken_at = datetime.fromtimestamp(media.taken_at) if isinstance(
            media.taken_at, int) else media.taken_at

        posts.append(Media(
            id=media.pk,
            # Use media_url if available, otherwise fallback to thumbnail_url
            url=str(media.media_url) if hasattr(
                media, "media_url") and media.media_url else str(media.thumbnail_url),
            caption=media.caption_text or "",
            media_type=media.media_type,
            taken_at=taken_at,
            like_count=media.like_count,
            comment_count=media.comment_count,
            video_url=str(media.video_url) if hasattr(
                media, "video_url") and media.video_url else None,
            permalink=permalink
        ))
    return posts
...
def get_stories(username: str = Query(..., description="Instagram username to fetch stories from")):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving stories: {e}")
    ...
    for story in stories:
        ...
        taken_at = datetime.fromtimestamp(story.taken_at) if isinstance(
            story.taken_at, int) else story.taken_at
        expiring_at = None
